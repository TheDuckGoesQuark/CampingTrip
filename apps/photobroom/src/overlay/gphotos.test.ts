...
function cell(opts: {
  ...
  const cb = checkbox
    ? `<div role="checkbox" aria-label="${label}" aria-checked="false"></div>`
    : "";
  return `
...
describe("id + thumbnail parsing", () => {
  ...
  it("falls back to an inline background-image when the attr is absent", () => {
    document.body.innerHTML = cell({
      id: "x",
      bgStyle: "https://photos.fife.usercontent.google.com/pw/styled=w100-h100-no",
    });
  ...
  it("upscales the size directive but keeps trailing flags + query", () => {
    expect(upscale("https://h/pw/x=w81-h177-no?authuser=0")).toBe(
      "https://h/pw/x=w640-h640-no?authuser=0",
    );
    expect(upscale("")).toBe("");
...
describe("readGrid", () => {
  it("reads id, upscaled thumbnail, and aria-label from a realistic grid", () => {
    document.body.innerHTML =
      cell({
        id: "AAA",
        thumb: "https://h/pw/a=w81-h177-no",
        label: "Photo – Portrait – 29 Jun 2024, 05:23:03",
      }) +
      cell({
        id: "BBB",
        thumb: "https://h/pw/b=w82-h177-no",
        label: "Photo – Landscape – 30 Jun 2022, 19:36:56",
      });
