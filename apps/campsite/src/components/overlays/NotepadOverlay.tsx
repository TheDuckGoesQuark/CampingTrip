...
export default function NotepadOverlay() {
  ...
  // Reset to the first page each time it opens.
  useEffect(() => {
    if (notepadFocused) setPage(0);
  }, [notepadFocused]);
  ...
  // Arrow keys flip pages (Escape is handled by the dialog → onOpenChange).
  useEffect(() => {
  ...
  return (
    <Modal
      ...
      open={notepadFocused}
      onOpenChange={(open) => {
        if (!open) navigate(routes.tent);
      }}
      ariaLabel="Notepad — journal entries"
    >
      <div
        style={{
          position: "absolute",
      ...
      >
        ...
        <NavArrow
          direction="right"
          disabled={page === notebookEntries.length - 1}
          onClick={goNext}
        />
