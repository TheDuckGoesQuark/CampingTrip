          ...
          {view === "list" ? (
            <SongList
              onSelect={(i) => {
                ...
                useMusicStore.getState().setTrack(i);
              }}
            />
          ) : (
            <NowPlaying />
...
function NowPlaying() {
  ...
  const wrappedIndex =
    songs.length > 0 ? ((currentTrackIndex % songs.length) + songs.length) % songs.length : 0;
  ...
  return (
    ...
    >
      ...
      >
        ...
        >
          <span style={{ fontSize: 32, opacity: 0.5 }}>{isPlaying ? "♪" : "♫"}</span>
        </div>
  ...
  return (
    <div
      style={{
        ...
        boxShadow:
          "inset 0 2px 4px rgba(255,255,255,0.3), inset 0 -2px 4px rgba(0,0,0,0.2), 0 2px 8px rgba(0,0,0,0.15)",
      }}
    >
      {/* Quadrant buttons */}
