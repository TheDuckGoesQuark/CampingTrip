...
describe("mouse input mapping", () => {
  it("mouse coordinates map to -0.5 to 0.5 range", () => {
    ...
    const mapX = (clientX: number) => clientX / windowWidth - 0.5;

    expect(mapX(0)).toBeCloseTo(-0.5);
    expect(mapX(windowWidth / 2)).toBeCloseTo(0);
    expect(mapX(windowWidth)).toBeCloseTo(0.5);
  });
});
