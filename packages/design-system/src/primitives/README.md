...
| `Menu.tsx`       | Re-exports the `Menu` namespace from `@base-ui/react/menu` (`.Root`, `.Trigger`, `.Portal`, `.Positioner`, `.Popup`, `.Item`, `.Separator`, …). Backs `../components/desktop/MenuBar`'s pull-downs.                                                                                                                                          |
| `Field.tsx`      | Re-exports the `Field` namespace from `@base-ui/react/field` (`.Root`, `.Label`, `.Control`, `.Description`, `.Error`, `.Validity`). Supplies label/control association and the `aria-describedby` + `aria-invalid` wiring. Backs `../components/form`.                                                                                      |
| `Popover.tsx`    | Re-exports the `Popover` namespace from `@base-ui/react/popover` (`.Root`, `.Trigger`, `.Portal`, `.Positioner`, `.Popup`, …). Backs `../components/desktop/MenuBar`'s control panels, where a `menu` role would strip the controls inside of their own semantics.                                                                           |
| `ToggleGroup.ts` | Re-exports `ToggleGroup` from `@base-ui/react/toggle-group` and `Toggle` from `@base-ui/react/toggle` — two packages in one shim, since a `Toggle` outside a group is a control the DS does not expose. Supplies the roving focus and arrow-key movement across a set of mutually exclusive choices. Backs `../components/SegmentedControl`. |
